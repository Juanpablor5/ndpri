<?php
	...
	if (isset($_POST['crearform'])) {
		...
		header('Location: dp_def01.php?idtipform='.$idtipform.'&idf='.$idformato.'');
	...
	if (isset($_POST['modifform'])) {
		...
		header('Location: dp_def01.php?idtipform='.$idtipform.'&idf='.$idf.'');
		...
		if ($idtipform > 0) {
			$sqlins = "INSERT INTO dp_def01 
					  ...
					  ( $idf,
						...
						$codempl,
						NOW() )";
			$rstins = mysql_query($sqlins) or die ('Error INSERT dp_def01 : ' . mysql_error());
		}
	...
	if (isset($_POST['verificar'])) {
		...
		if (!empty($a)){
		  ...
		  $sqlapr = "UPDATE dp_def01 SET cod_emp2 = ".$codempl.", fec_veri = NOW() WHERE id = ".$txtnum1;
		  echo $sqlapr;
